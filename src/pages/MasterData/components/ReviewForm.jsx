...
import { getMasterDataItemsByMasterDataId } from '@/services/masterData';
...
const ReviewForm = (props) => {
  ...
  const {
    onSubmit: handleCopy,
    onCancel: handleReviewModalVisible,
    reviewModalVisible,
    values,
  } = props;
  ...
  const renderFooter = () => {
    return (
      <div>
        ...
        >
          {intl.formatMessage({ id: 'page.modal.copyBtn' })}
        </Button>
  ...
  const columns = [
    {
      ...
      render: (text, row, index) => {
        return row.display_name === fieldName.header || row.display_name === fieldName.item ? (
          {
            children: <span style={{ fontWeight: 'bold' }}> {row.display_name}</span>,
            props: {
              colSpan: 5,
            },
          }
      ...
      tempMasterWHTField.forEach((item) => {
        if (
          ...
          addWHTField.forEach((itemField) => {
            ...
            if (refIndex !== -1) {
              newMasterField.splice(refIndex, 0, itemField);
            }
          });
  ...
  const getNumberStyleName = (id) => {
    let filterList = numberStyleList.filter((item) => item.id === id);
  ...
  return (
    <Modal
      ...
      destroyOnClose
      maskClosable={false}
      title={<FormattedMessage id="page.modal.reviewMasterDataTitle" />}
    ...
    >
      <Card>
        <h4>
          {intl.formatMessage({ id: 'page.modal.documentType' })} : <span style={{ marginLeft: 6 }} >{document_name}</span>
        </h4>
        <h4>
          {intl.formatMessage({ id: 'page.modal.vendor' })} :{' '}
          <span style={{ marginLeft: 58 }}>{vendor_name}</span>
        </h4>
        <h4>
          {intl.formatMessage({ id: 'page.modal.additionalInfo' })} :{' '}
          <span style={{ marginLeft: 16 }}>{additional_info}</span>
        </h4>
        <h4>
          {intl.formatMessage({ id: 'page.modal.numberStyle' })} :{' '}
          <span style={{ marginLeft: 17 }}>{getNumberStyleName(numberStyleId)}</span>
        </h4>
        <Row>
            <Col span={2}> <h4> {intl.formatMessage({ id: 'page.modal.aiPrompt' })} :{' '} </h4> </Col>
            <Col span={21}  style={{ marginLeft: 13 }}> <h4><pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflow: 'hidden' }} >{ai_prompt || 'Not found AI Prompt'}</pre></h4> </Col>
        </Row>
        <h4>
          {intl.formatMessage({ id: 'page.modal.numberStyle' })} :{' '}
          <span style={{ marginLeft: 16 }}>{day_auto_duedate || 30} Day</span>
        </h4>
        <br />
