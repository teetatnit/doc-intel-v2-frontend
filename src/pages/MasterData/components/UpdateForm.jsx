...
import { getMasterDataItemsByMasterDataId, updateMasterData } from '@/services/masterData';
...
const UpdateForm = (props) => {
    ...
    return (
      <div>
        <Button
          ...
          loading={saving}
        >
          {saving ? 'Saving' : intl.formatMessage({ id: 'page.modal.saveBtn' })}
        </Button>
  ...
  const document_name = props.values.document_name;
  const vendor_code = props.values.vendor_code;
  const vendor_name = props.values.vendor_name;
  const [ai_prompt, setAI_Prompt] = useState(props.values.ai_prompt);
  const [day_auto_duedate, setAuto_duedate] = useState(props.values.day_auto_duedate);
  const [additional_info, setAdditional_Info] = useState(props.values.additional_info);
  ...
  const onInputAIPromptChange = (e) => {
    setAI_Prompt(e.target.value);
  };

  const onInputAdditionalInfoChange = (e) => {
    setAdditional_Info(e.target.value);
  };
  const onInputAutoDueDate = (e) => {
    setAuto_duedate(e);
  };
  ...
  const onSelectCompanyChange = (key, index) => (e) => {
    ...
    if (e === undefined) {
      ...
      getBeneficiariesListByVendorCode(vendor_code, e).then((result) => {
  ...
  const onFinish = (values) => {
    ...
    setSaving(true);
    message.loading({
      content: 'Updating Master Data...',
      key: 'updateMasterDataMsg',
      duration: 0,
    });
    const updateMasterField = masterField.filter(
      (item) => item.display_name !== fieldName.WHTAction,
    );
    const deleteMasterField = masterWHTField.filter((item) => item.item_id !== null);
    updateMasterData(
      masterdata_id,
      document_code,
      vendor_code,
      ...
      if (res && res.status === 'success') {
        form.resetFields();
        message.success({
          content: 'Update Master Data Successfully',
          key: 'updateMasterDataMsg',
          duration: 1,
        ...
        message.error({
          content: `Failed to Update Master Data. Reason: ${res.message}.`,
          key: 'updateMasterDataMsg',
          duration: 2,
        });
        setSaving(false);
        ...
        message.error({
          content: 'Failed to Update Master Data. Reason: Unknown Error',
          key: 'updateMasterDataMsg',
          duration: 2,
        });
        setSaving(false);
  ...
  return (
    <Modal
      ...
      destroyOnClose
      maskClosable={false}
      title={<FormattedMessage id="page.modal.editMasterDataTitle" />}
    ...
    >
      <Card>
        <h4>
          {intl.formatMessage({ id: 'page.modal.documentType' })} : <span style={{ marginLeft: 5 }}>{document_name}</span>
        </h4>
        <h4>
          {intl.formatMessage({ id: 'page.modal.vendor' })} :{' '}
          <span style={{ marginLeft: 60 }}>{vendor_name}</span>
        </h4>
        ...
        <Form form={form} layout="horizontal" onFinish={onFinish}>
          ...
          >
            <Input
              style={{ width: '400px', marginLeft: 14 }}
              ...
              disabled={loading}
              onChange={onInputAdditionalInfoChange}
          ...
          >
            <TextArea
              style={{ maxWidth: '1000px', marginLeft: 37 }}
              placeholder={`${intl.formatMessage({
                id: 'page.modal.input',
              })} ${intl.formatMessage({ id: 'page.modal.aiPrompt' })}`}
              disabled={loading}
              rows={4}
              onChange={onInputAIPromptChange}
              value={ai_prompt}
            />
          </Form.Item>
          <Form.Item
          ...
          >
            <InputNumber
              style={{ width: '200px' }}
              defaultValue={day_auto_duedate === null || day_auto_duedate === '' ? 30 : day_auto_duedate}
              disabled={loading}
              min = {0}
              max = {100}
              onChange={onInputAutoDueDate}
              placeholder={`${intl.formatMessage({ id: 'page.modal.input' })} ${intl.formatMessage({
