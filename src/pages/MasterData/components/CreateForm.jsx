...
import { createMasterData, getDefaultAIPrompt } from '@/services/masterData';
import { getDocumentsList } from '@/services/document';
import { getVendorsList } from '@/services/vendor';
import { getMasterFieldsList } from '@/services/masterField';
...
import { getApproversGdcByStartWithName, getApproversDbByStartWithName } from '@/services/approver';
import { getReviewersDbByStartWithName } from '@/services/reviewer';
import { getRequestersDbByStartWithName } from '@/services/requester';
...
const CreateForm = (props) => {
  ...
  const { onCancel: handleCreateModalVisible, createModalVisible, values } = props;
  ...
  function getMasterDataForEachFields() {
    getMasterFieldsList().then((response) => {
      ...
      const paymentTypesList = []; // getPaymentTypesList();
      const grApprovalForsList = []; // getGRApprovalForsList();
      ...
      Promise.all([
        ...
        setBankCharge(data[14]);
        setPaidFor(data[15]);
        setPreAdvice(data[16]);
        setRemittedCurrency(data[17]);
        setNumberStyleList(data[18]);
        setDefaultData(newMasterField);
      });
    });
  }
  ...
  const setDefaultData = async (data) => {
    form.setFieldsValue({
      document_code: "10",
      vendor_code: "0011001747",
      day_auto_duedate: "30",
      ai_prompt: await getDefaultAIPrompt()
    });

    const company = data.find(item=>{
      return item.display_name === 'Company'
    })

    if (company && company.default_value !== "") {
      await getGRApprovalForsListByCompanyCode(company.default_value).then((result) => {
        setGrApprovalFor(result);
      });

      await getPaymentTypesListByCompanyCode(company.default_value).then((data) => {
        setPaymentType(data);
      });

      await getBeneficiariesListByVendorCode(form.getFieldValue('vendor_code'), company.default_value).then((result) => {
        setBeneficiary(result);
      });
    }
  ...
  const onFinish = (values) => {
    ...
    setSaving(true);
    message.loading({
      content: 'Creating New Master Data...',
      key: 'createMasterDataMsg',
      duration: 0,
    });

    values.model_id = '';
    values.model_template_id = '';
    const craeteMasterField = masterField.filter(
      (item) => item.display_name !== fieldName.WHTAction,
    );
    createMasterData(values, craeteMasterField, 'Create').then((res) => {
      if (res && res.status === 'success') {
        form.resetFields();
        getMasterFields();
        message.success({
          content: 'Create New Master Data Successfully',
          key: 'createMasterDataMsg',
          duration: 1,
          onClose: handleCreateModalVisible(false),
        });
        setSaving(false);
      } else if (res) {
        message.error({
          content: `Failed to Create New Master Data. Reason: ${res.message}.`,
          key: 'createMasterDataMsg',
          duration: 2,
        });
        setSaving(false);
        ...
        message.error({
          content: 'Failed to Create New Master Data. Reason: Unknown Error',
          key: 'createMasterDataMsg',
          duration: 2,
        });
        setSaving(false);
  ...
  return (
    <Modal
      ...
      destroyOnClose
      maskClosable={false}
      title={<FormattedMessage id="page.modal.addMasterDataTitle" />}
